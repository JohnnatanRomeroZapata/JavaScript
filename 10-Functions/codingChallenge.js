...
const poll = {
  ...
  arrayAnswers: new Array(4).fill(0),
  ...
  registerNewAnswer() {
    ...
    // In case of AND, the expression is evaluated until we get one false result
    ...
    typeof answer === 'number' &&
      answer < this.arrayAnswers.length &&
      this.arrayAnswers[answer]++;
  ...
  displayResult(type = 'string') {
    if (type === 'string') {
      console.log(`Poll results are: ${this.arrayAnswers.join(',')}`);
    } else {
...
poll.displayResult.call({ arrayAnswers: [5, 2, 3] }, 'string');
poll.displayResult.call({ arrayAnswers: [1, 5, 3, 9, 6, 1] }, 'string');
